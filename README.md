...
The pack is **self-contained** — `engine/`, `packages/` (vendored `ltx-core`/`ltx-trainer`), nodes, and the installer all live inside the `O2noor-comfyui-ltx25-tile-train-Beta` folder. Nothing is hardcoded to any machine.

1. **Clone** this repo.
2. **Install ComfyUI** (comfy.org).
3. **Copy the node pack** into ComfyUI's `custom_nodes/`:
   ```bat
   xcopy /E custom_nodes\O2noor-comfyui-ltx25-tile-train-Beta D:\ComfyUI\custom_nodes\O2noor-comfyui-ltx25-tile-train-Beta\
   ```
4. **Run the installer from inside that folder** (creates the engine venv, installs deps, writes `config.json`):
   ```bat
   cd D:\ComfyUI\custom_nodes\O2noor-comfyui-ltx25-tile-train-Beta
   python install.py
   ```
   If you'd rather install in-place from the repo (copies the pack into a target `custom_nodes`):
   ```bat
   python custom_nodes\O2noor-comfyui-ltx25-tile-train-Beta\install.py --install-dir D:\ComfyUI\custom_nodes
   ```
...
### Multi-axis activation tiling
Extending the tiling system toward **2×2, 3×3 … 6×6 spatial tiling** with configurable overlap, to reduce activation memory further and allow larger training resolutions. **Experimental, not yet stable.**
