...
def _move_batch_to(batch: dict, device: torch.device) -> dict:
    """Recursively move tensors in a nested batch dict to *device* (TDP)."""
    out: dict = {}
    for k, v in batch.items():
        if isinstance(v, torch.Tensor):
            out[k] = v.to(device, non_blocking=True)
        elif isinstance(v, dict):
            out[k] = _move_batch_to(v, device)
        else:
            out[k] = v
    return out
...
class LtxvTrainer:
        ...
        with progress:
            ...
            for step in range(remaining_steps * cfg.optimization.gradient_accumulation_steps):
                ...
                with self._accelerator.accumulate(self._transformer):
                    ...
                    if os.environ.get("LTX_TDP") == "1":
                        # TDP: sum LoRA gradients across ranks (CPU collective,
                        # gloo-safe) so every rank's optimizer sees the full data.
                        from ltx_trainer.tdp_train_wrapper import tdp_sync_grads

                        tdp_sync_grads(self._transformer)
                        self._accelerator.wait_for_everyone()
    ...
    def _maybe_build_tdp_wrapper(self, batch: dict) -> None:
        """Custom: lazily wrap the (DDP-prepared) transformer in the grad-aware
        tiled-data-parallel wrapper once the first batch reveals the latent shape.

        Enabled by ``LTX_TDP=1``. Each training rank processes a spatial tile of
        the video; an autograd-aware all_reduce blends them so the loss sees the
        full video and DDP syncs the (LoRA-only) parameter gradients. This is a
        true per-step compute split (3x-style speedup across tile ranks) and is
        the official-engine equivalent of the jaimitoes multi-GPU goal.
        """
        if os.environ.get("LTX_TDP") != "1":
            return
        if getattr(self, "_tdp_built", False):
            return
        vdata = batch.get("video_latents")
        if not vdata or "height" not in vdata:
            logger.warning("LTX_TDP: no video_latents metadata in batch; skipping TDP wrap.")
            return

        from ltx_core.components.patchifiers import VideoLatentPatchifier
        from ltx_core.model.transformer.modality import Modality  # noqa: F401
        from ltx_core.tiling import DimensionTilingConfig, TileCountConfig
        from ltx_core.tools import VideoLatentTools
        from ltx_core.types import VideoLatentShape

        num_frames = int(vdata["num_frames"][0].item())
        height = int(vdata["height"][0].item())
        width = int(vdata["width"][0].item())
        fps = float(vdata.get("fps", [24.0])[0].item() if vdata.get("fps") is not None else 24.0)
        sf = self._training_strategy.video_scale_factors
        latent_shape = VideoLatentShape(
            batch=1,
            channels=128,
            frames=(num_frames - 1) // sf.time + 1,
            height=height // sf.height,
            width=width // sf.width,
        )
        ...
        # 2 training ranks -> 1x2 spatial grid (width split). Overlap 5 like the
        # official inference pipelines.
        tiling = TileCountConfig(
            height=DimensionTilingConfig(num_tiles=1, overlap=0),
            width=DimensionTilingConfig(num_tiles=2, overlap=5),
        )
        from ltx_trainer.tdp_train_wrapper import GradTiledDataParallelModelWrapper

        train_group = [int(x) for x in os.environ.get("LTX_TRAIN_RANKS", "0,1").split(",")]
        self._transformer = GradTiledDataParallelModelWrapper(
            self._transformer,
            video_tools=tools,
            tiling=tiling,
            train_group=train_group,
        )
        self._tdp_built = True
        logger.info(
            f"LTX_TDP: wrapped transformer for tiled data parallel "
            f"(latent {latent_shape.frames}x{latent_shape.height}x{latent_shape.width}, "
            f"tiles=1x2, fps={fps}, train_ranks={train_group})"
        )

    def _training_step(self, batch: dict[str, dict[str, Tensor]]) -> TrainingStepOutput:
        """Perform a single training step using the configured strategy."""
        self._maybe_build_tdp_wrapper(batch)
        if os.environ.get("LTX_TDP") == "1":
            # Without accelerator.prepare, the dataloader yields CPU tensors;
            # move the whole batch to the rank's GPU for TDP.
            batch = _move_batch_to(batch, self._accelerator.device)
    ...
    def _load_models(self) -> None:
        ...
        Custom order for this 32GB-RAM rig: the embeddings processor (~3.2B
        params, ~6GB) is loaded FIRST while host RAM is still free, then the
        ~44GB bf16 transformer streams in (paging to disk). Loading the
        transformer first would re-read its 44GB file for the connector keys
        under full memory pressure and crash with "invalid python storage".
        """
        ...
        logger.debug("Loading embeddings processor (before transformer; RAM-safe order)...")
    ...
    def _prepare_models_for_training(self) -> None:
        ...
        # noinspection PyTypeChecker
        if os.environ.get("LTX_TDP") == "1":
            # TDP: each rank holds the full model and processes a tile; DDP's
            # automatic CUDA gradient sync hangs under gloo on Windows, and it
            # would shard the batch (TDP needs both ranks on the SAME sample).
            # Gradients are synced manually on CPU after backward instead.
            # IMPORTANT: accelerator.prepare would move the model to GPU; without
            # it we must move it ourselves (quantize_model leaves it on CPU).
            device = self._accelerator.device
            self._transformer = self._transformer.to(device)
            logger.info(f"LTX_TDP: moved transformer to {device} (manual CPU grad sync)")
        else:
    ...
    def _init_dataloader(self) -> None:
        ...
        # TDP needs every rank to see the SAME batch (each processes a different
        # tile of it), so bypass accelerate's data sharding in that mode.
        if os.environ.get("LTX_TDP") == "1":
