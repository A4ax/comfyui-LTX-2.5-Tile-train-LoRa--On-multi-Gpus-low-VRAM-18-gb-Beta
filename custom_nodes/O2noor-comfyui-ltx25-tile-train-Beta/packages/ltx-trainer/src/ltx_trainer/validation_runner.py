...
def _local_rank_device() -> torch.device:
    ...
    Custom: ``LTX_VALIDATION_DEVICE`` (e.g. "cuda:2") routes all validation-side model
    work (Gemma-4 text encoder, VAE encoder/decoder, vocoder) to a dedicated GPU so it
    never contends with the FSDP training pair's VRAM. This is the 3-GPU role-split:
    training on 2x12GB 3060s, validation/preprocess on the 8GB 4060.
    """
    if not torch.cuda.is_available():
        return torch.device("cpu")
    override = os.environ.get("LTX_VALIDATION_DEVICE")
    if override:
