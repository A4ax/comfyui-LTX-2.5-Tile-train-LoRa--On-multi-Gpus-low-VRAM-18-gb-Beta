...
dist.init_process_group("gloo", init_method="tcp://127.0.0.1:29570", rank=rank, world_size=world)
...
def main():
    ...
    DATASET_ROOT = cfg.get("dataset_root") or engine_env.DATASET_ROOT
    ...
    for step in range(N_STEPS):
        ...
        # ---- per-tile forward through sharded ranks; retain x_in/xh for backward ----
        tile_outs = []
        tile_bases = []
        fwd_xins = []
        fwd_xhs = []
        ...
        for ti, tmod in enumerate(tiles):
            ...
            if rank == 0:
                ...
                x_in = torch.empty_like(base.x).cpu()
            ...
            if IS_LAST := (rank == world - 1):
                ...
                if audio_base is not None and ah is not None:
                    ...
                    audio_outs.append(aout)
            else:
                dist.send(xh.detach().clone().cpu().contiguous(), rank + 1)
                if audio_base is not None and ah is not None:
                    dist.send(ah.detach().clone().cpu().contiguous(), rank + 1)

        loss_v = None
        loss_a = None
        if rank == world - 1:
            full_out = blend_tiles(helper, tile_outs, ctxs)
            loss_v = torch.nn.functional.mse_loss(full_out, target)
            loss_a = None
            if audio_base is not None and audio_outs and target_a is not None:
                # Audio is NOT tiled -> a single full output; don't blend via the
                # video tiling helper (their token counts differ).
                full_audio = audio_outs[0]
                loss_a = torch.nn.functional.mse_loss(full_audio, target_a)
            loss = loss_v if loss_a is None else loss_v + loss_a
            torch.cuda.synchronize(device)
        dist.barrier()

        # ---- backward: thread grads back per tile (uses retained x_in/xh) ----
        # Last rank: ONE loss.backward() walks the graph once (single checkpoint
        # recompute) and fills every tile's x_in.grad + all last-rank LoRA grads.
        # Middle ranks: one autograd.grad per tile. Keeps checkpointing so VRAM
        # stays low while cutting the old 4x retain_graph backward recompute.
        if rank == world - 1:
            loss.backward()
            for ti in range(len(tiles)):
                ...
                if rank > 0:
                    dist.send(g_in.detach().float().cpu().contiguous(), rank - 1)
                if audio_base is not None:
                    a_in = fwd_a_xins[ti]
                    g_a = a_in.grad if a_in.grad is not None else torch.zeros_like(a_in)
                    dist.send(g_a.detach().float().cpu().contiguous(), rank - 1)
                dist.barrier()
        else:
            for ti in range(len(tiles)):
                x_in = fwd_xins[ti]
                xh = fwd_xhs[ti]
                g = torch.zeros_like(xh, dtype=torch.float32).cpu()
                ...
                if rank > 0:
                    ...
                    dist.send(d_in.detach().float().cpu().contiguous(), rank - 1)
                # Audio gradient channel (voice+face).
                if audio_base is not None:
                    a_in = fwd_a_xins[ti]
                    ah = fwd_a_xhs[ti]
                    ga = torch.zeros_like(ah, dtype=torch.float32).cpu()
                    ...
                    if rank > 0:
                        ...
                        dist.send(d_a.detach().float().cpu().contiguous(), rank - 1)
                dist.barrier()
        ...
        peak_local = torch.tensor([torch.cuda.max_memory_allocated(device) / 1e9])  # CPU (gloo-safe)
        peak_all = [torch.zeros(1) for _ in range(world)]
